...
def test_3_4(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    answer = round(answer.sum(), 1)
    assert answer['test_neg_mean_squared_error'] == -418.7, "The range of your test negative mean squared error is incorrect. Are you fitting the model properly?"
    assert answer['train_neg_mean_squared_error'] == -398.7, "The range of your training negative mean squared error is incorrect. Are you fitting the model properly?"
    assert answer['test_neg_root_mean_square'] == -43.6, "The range of your test negative root mean squared error is incorrect. Are you fitting the model properly?"
    assert answer['train_neg_root_mean_square'] == -44.5, "The range of your training negative root mean squared error is incorrect. Are you fitting the model properly?"
    assert answer['test_r2'] == 2.9, "The range of your test r2 is incorrect. Are you fitting the model properly?"
    assert answer['train_r2'] == 2.9, "The range of your training r2 is incorrect. Are you fitting the model properly?"
