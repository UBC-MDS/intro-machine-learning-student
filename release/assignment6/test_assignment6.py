...
def test_2_1_3(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    answer = [x.lower() for x in sorted(answer)]
    assert sha1(str(answer).encode('utf8')).hexdigest() == 'd1d0a486a44f1a3d0d26511fc2a53383d706e287', "Your answer is incorrect. Are you analysing the dataframe correctly?"
    return("Success")
...
def test_3_2(answer):
    ...
    assert 'n_estimators=20' in str(list(answer)), "Make sure you are using a setting n_estimators to 20."
...
def test_3_7(answer):
    ...
    assert answer.get_params()['n_iter'] == 8, "Make sure you are iterating 8 times."
...
def test_5_12(answer):
    assert answer.shape == (10, 4), "The dimensions of you solution is incorrect. Are you using 10-fold cross validation?"
    assert sorted(list(answer.columns)) == ['fit_time', 'score_time', 'test_score', 'train_score'], "Your dataframe contains the incorrect columns. Are you setting up the mocel correctly?"
