...
def test_1_7(answer):
    ...
    assert answer.encoding.x.shorthand == 'style' or answer.encoding.x.field == 'style', "Make sure you are plotting 'style' on the x-axis."
    assert answer.encoding.y.shorthand == 'count()' or answer.encoding.y.field == 'count()', "Make sure you are plotting 'count()' on the y-axis."
    return ("Success")
...
def test_3_8(answer):
    ...
    assert sum([len(x) for x in answer.values()]) == 54, "Make sure you are uisng 10-fold cross validation"
...
def test_3_9(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert answer.shape == (36, 3), "The dimensions of your dataframe is incorrect. Are you tranforming the cv socres dictionary?"
...
def test_3_10(answer):
    ...
    assert 'n_neighbors' in answer.encoding.x.shorthand  or  'n_neighbors' in answer.encoding.x.field , "Make sure you are plotting 'n_neighbors' on the x-axis."
    assert 'accuracy' in answer.encoding.y.shorthand  or  'accuracy' in answer.encoding.y.field, "Make sure you are plotting 'accuracy' on the y-axis."
    assert not answer.title == 'Undefined', "Make sure you are providing a title for the plot."
    return("Success")

def test_3_11(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(str(answer).encode('utf8')).hexdigest() == "b85ab32eaa572c8016edf68011078dceed8149e5", "Your solution is incorrect. Please try again. The '.idmax' may be helpful here."
    return("Success")

def test_3_12(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(str(round(answer,2)).encode('utf8')).hexdigest() == "3531e3225cc01d9da74d8f4a7ca4b3138d2b7495", "Your test score is incorrect. Are you fitting the model properly?" 
    return("Success")
...
def test_4_2(answer):
    ...
    assert sum([len(x) for x in answer.values()]) == 21, "Make sure you are uisng 10-fold cross validation"
    assert min(answer['mean_cv_score']) > 0 and max(answer['mean_cv_score']) < 1, "The range of your cross validation scores is incorrect. Are you fitting the model properly?"
    assert min(answer['mean_train_score']) > 0 and max(answer['mean_train_score']) < 1, "The range of your training scores is incorrect. Are you fitting the model properly?"
    return("Success")
...
def test_4_3(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(str(answer).encode('utf8')).hexdigest() == "6ae529bf5a75a6207b2f277781b8e5f5f738d512", "Your answer is incorrect. Are you examining the results dictionary correctly? The idxmax() function might be useful here."
    return("Success")
...
def test_4_5(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(str(answer).encode('utf8')).hexdigest() == "48e1c024e47628701999b91e4327d4f495beeb29", "Your answer is incorrect. Are you examining the results dictionary correctly."
    return("Success")
...
def test_4_7(answer):
    ...
    assert sum([len(x) for x in answer.values()]) == 196, "Make sure you are uisng 5-fold cross validation"
...
def test_4_8(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert answer.shape == (49, 4), "The dimensions of your dataframe is incorrect. Are you tranforming the cv socres dictionary?"
    assert sum(sorted(answer['valid_accuracy'],reverse=True) == answer['valid_accuracy']) == 49, "Make sure you are sorting based on 'valid_accuracy' in descending order."
...
def test_4_9(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(str(round(answer,2)).encode('utf8')).hexdigest() == "715cf4cc66c307fa0a5e37e6f02176e1e11b1554", "Your answer is incorrect. Are you computing the best values correctly and using those to fit the model?"
    return("Success")
